...
static void computePositionEx_f16(fix16 *x2, fix16 *y2, fix16 x1, fix16 y1, fix16 ang, fix16 dist, fix16 cosMul, fix16 sinMul, bool useMul)
{
    ...
    if (useMul)
    {
        cosVal = F16_mul(cosVal, cosMul);
        sinVal = F16_mul(sinVal, sinMul);
    }
...
{
    computePositionEx_f16(x2, y2, x1, y1, ang, dist, FIX16(1), FIX16(1), FALSE);
}

void F16_computePositionEx(fix16 *x2, fix16 *y2, fix16 x1, fix16 y1, fix16 ang, fix16 dist, fix16 cosMul, fix16 sinMul)
{
    computePositionEx_f16(x2, y2, x1, y1, ang, dist, cosMul, sinMul, TRUE);
}
...
FORCE_INLINE fix16 V2D_F16_getAngle(V2f16* pt1, V2f16* pt2)
{
    return F16_getAngle(pt1->x, pt1->y, pt2->x, pt2->y);
}

FORCE_INLINE void V2D_F16_computePosition(V2f16* pt, fix16 ang, fix16 dist)
{
    computePositionEx_f16(&pt->x, &pt->y, pt->x, pt->y, ang, dist, FIX16(1), FIX16(1), FALSE);
}

FORCE_INLINE void V2D_F16_computePositionEx(V2f16* pt, fix16 ang, fix16 dist, fix16 cosAdj, fix16 sinAdj)
{
    computePositionEx_f16(&pt->x, &pt->y, pt->x, pt->y, ang, dist, cosAdj, sinAdj, TRUE);
}


FORCE_INLINE u32 V2D_S32_getApproximatedDistance(V2s32* v)
{
